"""
main_sqlite.py - Run the whole pipeline end to end against a sqlite database.

    population -> trees -> runs -> process -> evaluate

The same five steps as the text mode, over the same code -- but nothing is
scattered across run/ afterwards. The population, every setting the sweep ran
under, every seed, every generated script, every transcript and every score go
into one database, described in store.py:

    runs -> settings, individuals -> executions -> exchanges

Why a database
--------------
The text mode answers "what happened" by leaving a hundred files in run/, and
the next sweep overwrites them. This mode keeps sweeps side by side and lets you
ask across them:
...
The text mode leaves WEIGHT_SEED as None and redraws every execution, so there
the weights are recorded but the draw cannot be repeated. This is the one place
the two modes deliberately behave differently.
...
crashes is not a failure: it is a row in executions with its exit code, the same
as in the text mode.
...
Usage:
    python main_sqlite.py                    # a new sweep, every step
    python main_sqlite.py --list             # show the steps without running them
    python main_sqlite.py process evaluate   # resume the latest sweep
    python main_sqlite.py evaluate --run 3   # score sweep 3
    python main_sqlite.py --limit 3          # smoke test: only the first few
...
def step_trees(context):
    ...
    for row in rows:
        try:
            ...
            # Same as the text mode: an undrawable individual is recorded with
            # its complaint, not dropped.
...
def step_runs(context):
    ...
    for row in rows:
        ...
        source = generate_runs.render(
            ...
            script_name=name,
            provenance="Generated by main_sqlite.py from run %d, individual %d of %s."
...
def step_process(context):
    ...
    # exactly as in the text mode -- mocked scripts load nothing, and demanding
    # the venv for them would put a GPU-less machine out of reach.
...
def run(steps, context):
    ...
    Mirrors main_txt.run, and differs where the database does: a sweep that
    stops part way is left marked 'failed', so `python store.py --list` says so
    rather than presenting a half-finished run as a result.
    """
    started = time.time()
