...
Usage:
    python start_run.py                    # a new sweep, every step
    python start_run.py --list             # show the steps without running them
    python start_run.py --evaluators       # show the ways an answer can be scored
    python start_run.py population trees runs   # the fast half, before a model load
    python start_run.py process evaluate   # resume the latest sweep
    python start_run.py evaluate --run 3   # score sweep 3
    python start_run.py --limit 3          # smoke test: only the first few
...
def step_runs(context):
    ...
    for row in rows:
        ...
        source = generate_runs.render(
            ...
            script_name=name,
            provenance="Generated by start_run.py from run %d, individual %d of %s."
...
def step_selection(context):
    ...
    print("each copy is its parent field for field and the newcomer is bare -- "
          "re-derive their trees, scripts and seeds with: python start_run.py trees runs")
...
def context_for(conn, run_id, conf, args):
    ...
    each driver: start_run.py and continue_run.py both come through this, so a run
...
def main(argv=None):
    ...
    parser.add_argument("--next-generation", action="store_true",
                        help="also run %s -- the steps that build the next "
                             ...
                             "fitness, because start_run.py's one generation is the "
