...
An adopted sweep must hold no individuals yet -- main.py's half of the run draws
...
def prepared(options):
    ...
    the search. Handed one of those, `python full_run.py --db it.sqlite3` means
    ...
    in, exactly as before -- `full_run.py --db run_real/gep.sqlite3` goes on
...
def adopt(options, run=None):
    ...
    search. main.py then draws the population into it and takes it through its
    ...
    try:
        ...
        if held:
            raise SystemExit(
                "run %d in %s already holds %d individual(s). full_run.py starts "
...
def describe(options, run_id, conf, splits, asked):
    ...
    is worth a line: this is the one decision full_run.py makes on its own, and
...
def cli(argv=None):
    ...
    if generations < 1:
        raise SystemExit(
            "%d generation(s) for the second half; main.py's generation would be "
            "the whole run. Use main.py on its own for that." % generations)
    ...
    code = call(main.main, first + forwarded(options, bool(adopted)))
    if code:
        print()
    ...
    if not adopted:
        ...
        if run_id is None:
            print()
            print("main.py made no new sweep in %s; nothing to continue."
                  % options.db)
            return 1
...
def parse(argv):
    ...
    parser.add_argument("--generations", type=int, default=None, metavar="N",
                        help="generations for continue_run.py, on top of the one "
                             "main.py runs (default: with --run, the sweep's own "
